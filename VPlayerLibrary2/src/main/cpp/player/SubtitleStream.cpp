...
static void assCallback(int level, const char *fmt, va_list va, void *data) {
    if (level < 100) {
...
ASS_Image *SubtitleStream::getASSImage(double pts) {
    std::lock_guard<std::mutex> lk(mASSMutex);
    return ass_render_frame(mAssRenderer, mAssTrack, (long long int) pts, NULL);
}

bool SubtitleStream::isSSASubtitle() {
    if (!mCContext) {
        return false;
    }
    AVCodecID id = mCContext->codec_id;
    return id == AV_CODEC_ID_ASS || id == AV_CODEC_ID_SRT || id == AV_CODEC_ID_TEXT;
}


int SubtitleStream::synchronizeQueue(Clock* vclock) {
    // TODO see if this is needed or can merge with when we composite to video frame
    if (isSSASubtitle()) {
        return 0;
    }

    Frame *sp, *sp2;
    _log("Subtitles remaining %d", mQueue->getNumRemaining());
    while (mQueue->getNumRemaining() > 0) {
        ...
        if (mQueue->getNumRemaining() > 1) {
            if (!(sp2 = mQueue->peekNext())->subtitle()) {
                return error(AVERROR_INVALIDDATA, "Frame is not subtitle frame");
            }
        } else {
            sp2 = NULL;
        ...
        _log("---> %lf > (%lf + %f)", vclock->getPts(), sp->pts(), sp->endSubTimeMs())

        // Play next subtitle if seek or past it
        if (sp->serial() != getPacketQueue()->serial()
            || (vclock->getPts() > (sp->pts() + sp->endSubTimeMs()))
            || (sp2 && vclock->getPts() > (sp2->pts() + sp->startSubTimeMs()))) {
            // TODO do we need this?
            _log("Normally we add more stuff here, do we need to show subtitles?");
//            if (sp->hasUploaded) {
...
int SubtitleStream::blendToFrame(AVFrame *vFrame) {
    if (isSSASubtitle()) {
        ASS_Image* image = getASSImage(vFrame->pts);
        for (; image != NULL; image = image->next) {
            blendSSA(vFrame, image);
        }
    } else {
    // TODO if image sub, read queue and blend it here
    //    for (int i = 0; i < subtitle->num_rects; i++) {
    //        AVSubtitleRect* rect = subtitle->rects[i];
    //
    //        rect->x = av_clip(rect->x, 0, frame->width);
    //        rect->y = av_clip(rect->x, 0, frame->height);
    //        rect->w = av_clip(rect->x, 0, frame->width - rect->x);
    //        rect->h = av_clip(rect->x, 0, frame->height - rect->y);
    //
    //        mSubContext = sws_getCachedContext(mSubContext, rect->w, rect->h, AV_PIX_FMT_PAL8,
    //                                           rect->w, rect->h, outFormat,
    //                                           0, NULL, NULL, NULL);
    //        if (mSubContext) {
    //            __android_log_print(ANDROID_LOG_ERROR, sTag,
    //                                "Cannot allocate conversion context for subtitles");
    //            return AVERROR(EINVAL);
    //        }
    //        if ((ret = sws_scale(mSubContext, (const uint8_t *const *) rect->data, rect->linesize,
    //                             0, rect->h, mBufferFrame->data, rect->linesize)) < 0) {
    //            __android_log_print(ANDROID_LOG_ERROR, sTag,
    //                                "Cannot convert subtitle rect to surface");
    //            return ret;
    //        }
    //    }
    }
...
int SubtitleStream::open() {
    ...
    if (!ret) {
        // Init the ass library
        if (isSSASubtitle()) {
            _log("Use ass library")
            ...
            ass_set_message_cb(mAssLibrary, assCallback, NULL);
            ...
            ass_set_frame_size(mAssRenderer, mRenderWidth, mRenderHeight);
            _log("Sub use %d %d", mRenderWidth, mRenderHeight)

            // Load font attachments from video into the engine
            for (int i = 0; i < mFContext->nb_streams; i++) {
                const AVStream *st = mFContext->streams[i];
                if (st->codecpar->codec_type == AVMEDIA_TYPE_ATTACHMENT && isFontAttachment(st)) {
                    const AVDictionaryEntry *tag = av_dict_get(st->metadata, "filename", NULL,
                                                               AV_DICT_MATCH_CASE);
                    if (tag) {
                        _log("Loading attached font: %s", tag->value);
            ...
            if (mCContext->subtitle_header_size > 13 && mCContext->subtitle_header) {
                if (!strncasecmp((char*) mCContext->subtitle_header, sAssScriptHeader, 13)) {
                    ass_process_data(mAssTrack, (char*) mCContext->subtitle_header,
                                     mCContext->subtitle_header_size);
                }
            }
...
void SubtitleStream::blendSSA(AVFrame *vFrame, const ASS_Image *subImage) {
...
int SubtitleStream::onProcessThread() {
    ...
    while (1) {
        ...
        if ((ret = decodeFrame(sp->subtitle())) < 0) {
            ...
            AVSubtitle* subtitle = sp->subtitle();
            switch (subtitle->format) {
                case 0: /* graphics */
                    sp->updateAsSubtitle(mCContext->width, mCContext->height, mPktSerial);
                    mQueue->push();
                    continue;
                case 1: /* text subs */
                    {
                ...
                default:
                    __android_log_print(ANDROID_LOG_WARN, sTag,
                                        "Does not support this type of subtitle %d",
                                        subtitle->format);
            }
